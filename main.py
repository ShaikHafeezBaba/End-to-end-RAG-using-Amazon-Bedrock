...
from langchain.txt_splitter import RecursiveCharacterTextSplitter
...
def get_documents():
    ...
    docs=(variable) docs: List[Document]
    return docs
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Send"):
            with st.spinner("Processing.."):
                faiss_index=FAISS.load_local("faise_local",bedrock_embedding,allow_dangerous_deserialization=True)
                llm=get_llm()
